...
export default function NmpaCheckPage() {
  ...
  const handlePayment = async () => {
    ...
    setError("");
    try {
      const reportId = `COSMETICS-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`;
      ...
      <div className="max-w-3xl mx-auto px-4 py-12">
        ...
        {step === "form" && (
          <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 sm:p-8 space-y-5">
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1.5">Brand Country / Origin</label>
              <input
                type="text"
